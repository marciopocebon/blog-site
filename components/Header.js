...
function Header() {
  ...
  useEffect(() => {
    if ((new Date().getHours() === 0, 1, 2, 3, 19, 20, 21, 22, 23, 24)) {
      setTimeframe("Night");
    } else if ((new Date().getHours() === 4, 5, 6, 7, 8, 9, 10, 11, 12)) {
      setTimeframe("Morning");
    }
  }, []);

  return (
    <div className="pt-10 justify-between flex sm:pt-11">
      <div className="hidden sm:flex">
        <svg
          height="36px"
          viewBox="0 0 3925 610"
          className="pl-12 pr-4 border-r-[1px] border-gray-200"
        >
        ...
        <p className="font-extrabold pl-5 pt-1 text-xl">Good {timeframe}</p>
      </div>
